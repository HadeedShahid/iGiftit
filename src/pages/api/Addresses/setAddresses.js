...
export default async function handler(req,res){
    ...
    if (req.method==='POST'){
        ...
        const oldAddresses = await fetch(`${process.env.NEXT_PUBLIC_PROTOCOL}://${process.env.NEXT_PUBLIC_CUSTOM_URL}/api/Addresses/getAddresses`,options).then(async res=>{return (await res.json()).message});
